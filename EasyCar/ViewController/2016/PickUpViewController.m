...
- (void)viewDidLoad {
    ...
    if ([CGTool isMonkey]) {
        [self.nav setTitle:@"我要取车" leftText:@"返回" rightTitle:@"设置" showBackImg:YES];
...
- (void)btnClick2:(UIButton*)sender {
    ...
    PickUpNumViewController *pickUpNum = [[PickUpNumViewController alloc] init];
    pickUpNum.mapArr = _mapDic[@"UIcontent"][@"plcFrame"][_btnTag];
    pickUpNum.plcNum = _btnTag;
    NSLog(@"%@",_mapDic);
    [self.navigationController pushViewController:pickUpNum animated:YES];
}
-(void)checkUpParkingHostory
{
    NSDictionary *dic = [[NSUserDefaults standardUserDefaults] objectForKey:@"parkingList"];
    NSLog(@"pickUpDIC: %@",dic);
    if (dic) {
        _btnTag = (int)[dic[@"plcTag"] integerValue];
        UIButton *sender = [self.view viewWithTag:_btnTag + 120];
        _btnText = sender.currentTitle;
        _parkingNum.text = [NSString stringWithFormat:@"当前已选择%@车库",sender.currentTitle];
        _chooseShowLabel.text = [NSString stringWithFormat:@"已选择%@车库",sender.currentTitle];
        [_parkingNum setTextColor:[UIColor orangeColor]];
        _inputLabel.text = dic[@"parkingNum"];
        if (_ishistory) {
            PickUpNumViewController *pickUpNum = [[PickUpNumViewController alloc] init];
            pickUpNum.mapArr = _mapDic[@"UIcontent"][@"plcFrame"][_btnTag];
            pickUpNum.plcNum = _btnTag;
            pickUpNum.parkingNum = dic[@"parkingNum"];
            if ([[CGTool shareInitial] isMonkeyWIFI]) {
                [self.navigationController pushViewController:pickUpNum animated:YES];
        ...
        case 912://确认键
            if (_inputLabel.text.length == 3) {
                ...
                if (_isConnectMonkeyWIFI) {
                    ...
                    [plcData wantToTakingWithPlc:_btnTag andParkingSpace:_inputLabel.text andSuccess:^(id obj) {
                        [MBProgressHUD hideAllHUDsForView:self.view animated:YES];
                        NSString *objStr = obj;
                        [MBProgressHUD showResult:YES text:[NSString stringWithFormat:@"正在为您下放%@载车板",objStr] delay:5.0f];
                    } orFailed:^(id obj) {
                        [MBProgressHUD hideAllHUDsForView:self.view animated:YES];
                        [MBProgressHUD showResult:NO text:[NSString stringWithFormat:@"车库繁忙中"] delay:3.0f];
...
-(void)autoParkingCar
{
    ...
    if (autoBtn.alpha != 0) {
        ...
        [plcData wantToParkingWithPlc:_btnTag andSuccess:^(id obj) {
            [MBProgressHUD showResult:YES text:[NSString stringWithFormat:@"正在为您下放%@",_inputLabel.text] delay:5.0f];
        } orFailed:^(id obj) {
